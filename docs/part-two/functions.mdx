...
Um eine Funktion zu deklarieren nutzen wir folgen Syntax:
...
Zwischen den geschwungenen Klammern (`{}`) befindet sich der sogenannte body der Funktion, welcher unsere
Logik enthält - hier nur ein simples `console.log`.

```js
    function showMessage(something) {
        console.log(something)
        // more code
    }
```

`something` könnte hier zum beispiel eine dynamische Willkommensnachricht sein.
...
Damit zeigen wir ebenfalls den Hauptsinn von Funktionen: Wiederholten Code zu vermeiden.

Wenn ihr merkt, dass ihr Code wiederholt, ist das meist eine Hinweis darauf,
dass ihr Dinge in eine Funktion auslagern, oder Probleme auf eine
...
Generell solltet ihr es vermeiden, den Wert von outer Variablen in Funktionen zu ändern.
...
Die Funktion `showMessage` wird mit den `string`s gecalled, kopiert (in der Zeile mit \* und \*\*)
die Werte der Strings (`from` und `text` in der Reihenfolge wie sie
der Funktion übergeben werden) und stellt diese innerhalb der Funktion als lokale Variablen zur
Verfügung. Erst dann wird der body ausgeführt.
...
`"text missing"` wird nun jedes mal anstelle von `undefined` zugewiesen, falls kein zweites Argument
übergeben wird.
...
Fällen eine elegante Lösung um Alternativen für fehlende Werte zu verwenden.
...
Mit dem keyword `return` können wir gezielt Werte zurückgeben, beispielsweise bei Berechnungen:
...
Generell sollte eine Funktion nur das tun was ihr Name aussagt und nicht mehr.
    ...
    Dies hilft automatisch dabei euren Code zu fragmentieren und damit maintenance sowie
...
Es erleichtert maintenance, spätere Upgrades oder Verbesserungen wenn viele einzelne Funktionen
