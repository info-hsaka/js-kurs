...
Ebenfalls an die Mathematik angelehnt gibt es in JS Vergleichsoperatoren:
...
Es verhindert ungewollte Datentypkonvertierungen und verhält sich damit "intuitiver".
:::
...
`else` ist optional und bietet uns die Möglichkeit, Code nur dann auszuführen, wenn die Bedingung eines `if`s `false` war. `else` wird direkt an den `if`-Block angehängt.
...
Dieser Operator bekommt drei Operanden:
...
es keinen `truthy` Wert wird der letzte Wert aus der Operation genommen. Falls alle Werte `boolean`s
(also Aussagen) sind, wertet der OR-Operator also umgangssprachlich aus, ob irgendeine "a oder b oder c..."
der Aussagen stimmt.
...
Der AND-Operator wertet also aus, ob alles wahr ist ("a und b und c...").
...
```js
const pi = 3.14;
const piIsExactlyThree = (pi === 3);

if (!piIsExactlyThree) {
    console.log("PI ist doch nicht 3 :-(");
}
```

Umgangssprachlich negiert er also eine Aussage ("nicht a").
