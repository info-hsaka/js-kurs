...
while (iteration < 3) {
  ...
  itertation++; // this increases the variable by one
}
```
...
Generell muss bei `begin` keine Variable initialisiert, an dieser Stelle kann
...
der loop abgebrochen und wir geben die Summe aller addierten Zahlen aus `alert("Sum: " sum)`
...
:::caution `break/continue` and `?`
`break` und `continue` sind bestimmte keyworder in JS. Es handelt sich hier um keine expressions
die mit dem ternary operator getriggert werden können.

Solcher code wirft grundsätzlich einen syntax error:

```js
  (i > 6) ? console.log(i) : continue //continue ist hier nicht erlaubt
```

Falls ihr so etwas ausdrücken möchtet, bietet sich hier der `if` operator an.
